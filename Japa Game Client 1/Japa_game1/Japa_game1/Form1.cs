    ...
    public partial class Form1 : Form
    {
      ...
      /*  SqlConnection conn = new SqlConnection("user id=ads-user;" +
        ...
        private void displayCard()
        {
            ...
            dataGridView1.Columns[0].Name = "Detail";
            DataGridViewImageColumn img = new DataGridViewImageColumn();
            int imgIndex = dataGridView1.Columns.Add(img);
            dataGridView1.Columns[imgIndex].Name = "Image";

            dataGridView1.AllowUserToAddRows = true;
            listNum = p1Card.Count;
            for (int i = 0; i < listNum; i++)
            {
               // dataGridView1[0, i].Value = img;

                int index = p1Card[i] - 1;
        ...
        private void btnshuffle_Click(object sender, EventArgs e)
        {
            ...
            p1Card.Clear();
            p2Card.Clear();
            // first cards to player 1
            Boolean existingCardFlag = true;
            ...
            for (int j = 0; j < cardCnt/2; j++)
            {
                while (true)
                {
                    ...
                    cardNum = rnd.Next(1, cardCnt);
                    existingCardFlag = p1Card.Contains(cardNum);

                    if (existingCardFlag == false)
                    {
                        p1Card.Add(cardNum);           
                        break;
                    }
            ...
            for (int h = 0; h < cardCnt; h++)
            {
                    ...
                    foundFlag = p1Card.Contains(cards[h]);
                    ...
                    // p2 has the remaining cardCnt/2
                    // card[h] not in p1 so go to p2
                    // first add them into tmpCards arry in random location among cardCnt/2
                    if (foundFlag == false)
                    {
                        while (true)
                        {
                            cardNum = rnd.Next(1, (cardCnt / 2)+1);
                            if (tmpCards[cardNum - 1] == -1)
                            {
                                tmpCards[cardNum - 1] = cards[h];
                                break;
                            }
                        }
                    }
            ...
            // now fill in tmpCards into p2
            for (int h = 0; h < cardCnt / 2; h++)
            {
                    p2Card.Add(tmpCards[h]);
            }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            if (display1Flag && cardCnt != 0)
            {
                // also display tie box
                // first read from tblTie
                // add to tie list 
                // then show it in box
                readCardsFromTieTbl();
        ...
        private void lboxTie_SelectedIndexChanged(object sender, EventArgs e)
        {
